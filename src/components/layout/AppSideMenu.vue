...
<template>
  <transition name="sidebar-collapse">
    <el-menu router :default-active="$route.path" :collapse="collapse" class="sidebar-menu" unique-opened>
      <!-- 遍历顶层路由，递归渲染 -->
      <SideMenuItem v-for="route in menuRoutes" :key="route.name ?? route.path" :route="route" :collapse="collapse" />
    </el-menu>
  </transition>
</template>
